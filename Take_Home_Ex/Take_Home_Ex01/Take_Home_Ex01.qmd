...
-   **Opportunity**: identify **underserved subzones** (low KDE but reasonable demand/transport) as candidate areas for **decentralised growth**.
    ...
    ```{r}
    kde_biz56123sg_fb <- density(biz56123sg_ppp_km,
                                  sigma=0.6, 
                                  edge=TRUE,
                                  kernel="gaussian")
    kde_biz56123sg_ab <- adaptive.density(
      biz56123sg_ppp_km, 
      method="kernel")
    plot(kde_biz56123sg_ab)

    par(mfrow=c(1,2))
    plot(kde_biz56123sg_fb, main = "Fixed bandwidth")
    plot(kde_biz56123sg_ab, main = "Adaptive bandwidth")
    ```
    ...
    #### **3.3 First Order SPPA at the Planning Subzone Level**
...
#biz56123_ys_ppp = biz_56123_ppp [ys_owin]
#biz56123_amk_bs_ppp = biz_56123_ppp [amk_bs_owin]
#biz56123_jw_je_ppp = biz_56123_ppp [jw_je_owin]
#biz56123_dc_ppp = biz_56123_ppp [dc_owin]
...
library(plotly)   # make sure it’s loaded
...
csr_plot <- ggplot(L_sbw_csr_df, aes(r, obs-r))+
  ...
  geom_rug(data=L_sbw_csr_df[L_sbw_csr_df$obs > L_sbw_csr_df$hi,], sides="b", colour=colour[1])  +
  geom_rug(data=L_sbw_csr_df[L_sbw_csr_df$obs < L_sbw_csr_df$lo,], sides="b", colour=colour[2]) +
  geom_rug(data=L_sbw_csr_df[L_sbw_csr_df$obs >= L_sbw_csr_df$lo & L_sbw_csr_df$obs <= L_sbw_csr_df$hi,], sides="b", color=colour[3]) +
  theme_tufte()+
  ggtitle(title)
...
if (nrow(L_sbw_csr_df[L_sbw_csr_df$obs > L_sbw_csr_df$hi,])==0){ 
  ...
  }else if (nrow(L_sbw_csr_df[L_sbw_csr_df$obs >= L_sbw_csr_df$lo & L_sbw_csr_df$obs <= L_sbw_csr_df$hi,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text2, traces = 4) %>% 
      rangeslider()
  }else {
  ...
  if (nrow(L_sbw_csr_df[L_sbw_csr_df$obs >= L_sbw_csr_df$lo & L_sbw_csr_df$obs <= L_sbw_csr_df$hi,])==0){
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text1, traces = 4) %>% 
      rangeslider()
  } else{
...
colour=c("#0D657D","#ee770d","#D3D3D3")
csr_plot <- ggplot(L_amk_bs_df, aes(r, obs-r))+
  # plot observed value
  geom_line(colour=c("#4d4d4d"))+
  geom_line(aes(r,theo-r), colour="red", linetype = "dashed")+
  # plot simulation envelopes
  ...
  geom_rug(data=L_amk_bs_df[L_amk_bs_df$obs > L_amk_bs_df$hi,], sides="b", colour=colour[1])  +
  geom_rug(data=L_amk_bs_df[L_amk_bs_df$obs < L_amk_bs_df$lo,], sides="b", colour=colour[2]) +
  geom_rug(data=L_amk_bs_df[L_amk_bs_df$obs >= L_amk_bs_df$lo & L_amk_bs_df$obs <= L_amk_bs_df$hi,], sides="b", color=colour[3]) +
  theme_tufte()+
  ggtitle(title)
...
if (nrow(L_amk_bs_df[L_amk_bs_df$obs > L_amk_bs_df$hi,])==0){ 
  if (nrow(L_amk_bs_df[L_amk_bs_df$obs < L_amk_bs_df$lo,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text3, traces = 4) %>%
      rangeslider() 
  }else if (nrow(L_amk_bs_df[L_amk_bs_df$obs >= L_amk_bs_df$lo & L_amk_bs_df$obs <= L_amk_bs_df$hi,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text2, traces = 4) %>%
      rangeslider() 
  }else {
  ...
  if (nrow(L_amk_bs_df[L_amk_bs_df$obs >= L_amk_bs_df$lo & L_amk_bs_df$obs <= L_amk_bs_df$hi,])==0){
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text1, traces = 4) %>%
      rangeslider() 
  } else{
...
colour=c("#0D657D","#ee770d","#D3D3D3")
csr_plot <- ggplot(L_pg_sk_df, aes(r, obs-r))+
  # plot observed value
  geom_line(colour=c("#4d4d4d"))+
  geom_line(aes(r,theo-r), colour="red", linetype = "dashed")+
  # plot simulation envelopes
  ...
  xlab("Distance r (m)") +
  ylab("L(r)-r") +
  geom_rug(data=L_pg_sk_df[L_pg_sk_df$obs > L_pg_sk_df$hi,], sides="b", colour=colour[1])  +
  geom_rug(data=L_pg_sk_df[L_pg_sk_df$obs < L_pg_sk_df$lo,], sides="b", colour=colour[2]) +
  geom_rug(data=L_pg_sk_df[L_pg_sk_df$obs >= L_pg_sk_df$lo & L_pg_sk_df$obs <= L_pg_sk_df$hi,], sides="b", color=colour[3]) +
  theme_tufte()+
  ggtitle(title)
...
# the below conditional statement is required to ensure that the labels (text1/2/3) are assigned to the correct traces
if (nrow(L_pg_sk_df[L_pg_sk_df$obs > L_pg_sk_df$hi,])==0){ 
  if (nrow(L_pg_sk_df[L_pg_sk_df$obs < L_pg_sk_df$lo,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text3, traces = 4) %>%
      rangeslider() 
  }else if (nrow(L_pg_sk_df[L_pg_sk_df$obs >= L_pg_sk_df$lo & L_pg_sk_df$obs <= L_pg_sk_df$hi,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text2, traces = 4) %>%
      rangeslider() 
  }else {
...
} else if (nrow(L_pg_sk_df[L_pg_sk_df$obs < L_pg_sk_df$lo,])==0){
  if (nrow(L_pg_sk_df[L_pg_sk_df$obs >= L_pg_sk_df$lo & L_pg_sk_df$obs <= L_pg_sk_df$hi,])==0){
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text1, traces = 4) %>%
      rangeslider() 
  } else{
...
colour=c("#0D657D","#ee770d","#D3D3D3")
csr_plot <- ggplot(L_jw_je_df, aes(r, obs-r))+
  # plot observed value
  geom_line(colour=c("#4d4d4d"))+
  geom_line(aes(r,theo-r), colour="red", linetype = "dashed")+
  # plot simulation envelopes
  ...
  xlab("Distance r (m)") +
  ylab("L(r)-r") +
  geom_rug(data=L_jw_je_df[L_jw_je_df$obs > L_jw_je_df$hi,], sides="b", colour=colour[1])  +
  geom_rug(data=L_jw_je_df[L_jw_je_df$obs < L_jw_je_df$lo,], sides="b", colour=colour[2]) +
  geom_rug(data=L_jw_je_df[L_jw_je_df$obs >= L_jw_je_df$lo & L_jw_je_df$obs <= L_jw_je_df$hi,], sides="b", color=colour[3]) +
  theme_tufte()+
  ggtitle(title)
...
# the below conditional statement is required to ensure that the labels (text1/2/3) are assigned to the correct traces
if (nrow(L_jw_je_df[L_jw_je_df$obs > L_jw_je_df$hi,])==0){ 
  if (nrow(L_jw_je_df[L_jw_je_df$obs < L_jw_je_df$lo,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text3, traces = 4) %>%
      rangeslider() 
  }else if (nrow(L_jw_je_df[L_jw_je_df$obs >= L_jw_je_df$lo & L_jw_je_df$obs <= L_jw_je_df$hi,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text2, traces = 4) %>%
      rangeslider() 
  }else {
...
} else if (nrow(L_jw_je_df[L_jw_je_df$obs < L_jw_je_df$lo,])==0){
  if (nrow(L_jw_je_df[L_jw_je_df$obs >= L_jw_je_df$lo & L_jw_je_df$obs <= L_jw_je_df$hi,])==0){
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text1, traces = 4) %>%
      rangeslider() 
  } else{
...
colour=c("#0D657D","#ee770d","#D3D3D3")
csr_plot <- ggplot(L_dc_df, aes(r, obs-r))+
  # plot observed value
  geom_line(colour=c("#4d4d4d"))+
  geom_line(aes(r,theo-r), colour="red", linetype = "dashed")+
  # plot simulation envelopes
  ...
  xlab("Distance r (m)") +
  ylab("L(r)-r") + 
  geom_rug(data=L_dc_df[L_dc_df$obs > L_dc_df$hi,], sides="b", colour=colour[1])  +
  geom_rug(data=L_dc_df[L_dc_df$obs < L_dc_df$lo,], sides="b", colour=colour[2]) +
  geom_rug(data=L_dc_df[L_dc_df$obs >= L_dc_df$lo & L_dc_df$obs <= L_dc_df$hi,], sides="b", color=colour[3]) +
  theme_tufte()+
  ggtitle(title)
...
# the below conditional statement is required to ensure that the labels (text1/2/3) are assigned to the correct traces
if (nrow(L_dc_df[L_dc_df$obs > L_dc_df$hi,])==0){ 
  if (nrow(L_dc_df[L_dc_df$obs < L_dc_df$lo,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text3, traces = 4) %>%
      rangeslider() 
  }else if (nrow(L_dc_df[L_dc_df$obs >= L_dc_df$lo & L_dc_df$obs <= L_dc_df$hi,])==0){ 
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text2, traces = 4) %>%
      rangeslider() 
  }else {
...
} else if (nrow(L_dc_df[L_dc_df$obs < L_dc_df$lo,])==0){
  if (nrow(L_dc_df[L_dc_df$obs >= L_dc_df$lo & L_dc_df$obs <= L_dc_df$hi,])==0){
    ggplotly(csr_plot, dynamicTicks=T) %>%
      style(text = text1, traces = 4) %>%
      rangeslider() 
  } else{
...
## **Objective: Examine Infrastructure Proximity**
